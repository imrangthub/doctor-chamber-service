...
/*	public static String getBeanToJsonString(Object beanClass) {
	...
	public static String oracleBuildPaginatedQuery(String baseQuery, PaginationCriteria paginationCriteria) {
		...
		StringBuilder sb = new StringBuilder(" SELECT FILTERED_ORDERD_RESULTS_FINAL.* FROM (");
		sb.append(" SELECT FILTERED_ORDERD_RESULTS.*, ROWNUM rowNo FROM (");
		sb.append(" SELECT BASEINFO.* FROM ( #BASE_QUERY# ) BASEINFO #WHERE_CLAUSE#  #ORDER_CLASUE# ) FILTERED_ORDERD_RESULTS )");
		sb.append(" FILTERED_ORDERD_RESULTS_FINAL");
		sb.append(" WHERE FILTERED_ORDERD_RESULTS_FINAL.rowNo > #PAGE_NUMBER# AND FILTERED_ORDERD_RESULTS_FINAL.rowNo <= #PAGE_SIZE#");
		String finalQuery = null;
