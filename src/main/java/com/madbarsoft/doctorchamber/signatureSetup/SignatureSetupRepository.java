...
@Repository
@Transactional
public class SignatureSetupRepository extends BaseRepository {
	...
	public Response gridList(HttpServletRequest request) {
		...
		SignatureSetupEntity signatureSetupEntity = new SignatureSetupEntity();
		Long doctorNo = Long.parseLong(request.getParameter("doctorNo"));
		if(doctorNo != null)signatureSetupEntity.setDoctorNo(doctorNo);
		DataTableRequest dataTableInRQ = new DataTableRequest(request);
		Long totalRowCount = totalCount(signatureSetupEntity);
		DataTableResults<SignatureSetupEntity> dataTableResults = null;
		...
		response = baseList(typedQuery(signatureSetupEntity, dataTableInRQ));
	...
	public SignatureSetupEntity findById(Long id) {
		SignatureSetupEntity obj = new SignatureSetupEntity();
		obj.setId(id);
		Response response = baseFindById(criteriaQuery(obj));
		if (response.isSuccess()) {
			return (SignatureSetupEntity) response.getObj();
		}
		return null;
	}

	public Response save(SignatureSetupEntity reqObj) {
		return baseOnlySave(reqObj);
	}

	public Response update(SignatureSetupEntity reqObj) {
		SignatureSetupEntity obj = findById(reqObj.getId());
		if (obj != null) {
			obj.setSignatureTitle(reqObj.getSignatureTitle());
			obj.setDoctorName(reqObj.getDoctorName());
			obj.setDoctorDegree(reqObj.getDoctorDegree());
	...
	public Response detele(Long id) {
		SignatureSetupEntity obj = findById(id);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private CriteriaQuery criteriaQuery(SignatureSetupEntity filter) {
		initEntityManagerBuilderCriteriaQueryRoot(SignatureSetupEntity.class);
	...
	private List<Predicate> criteriaCodtion(SignatureSetupEntity filter, CriteriaBuilder builder,
			Root<SignatureSetupEntity> root) {
		...
		if (filter != null) {
			...
			if (filter.getSignatureTitle() != null && !filter.getSignatureTitle().isEmpty()) {
				Predicate condition = builder.like(root.get("signatureTitle"), filter.getSignatureTitle());
				p.add(condition);
			}
	...
	private TypedQuery typedQuery(SignatureSetupEntity filter, DataTableRequest dataTableInRQ) {
		init();
	...
	private Long totalCount(SignatureSetupEntity filter) {
		CriteriaBuilder builder = criteriaBuilder();
		CriteriaQuery<Long> criteriaQuery = longCriteriaQuery(builder);
		Root<SignatureSetupEntity> root = from(SignatureSetupEntity.class, criteriaQuery);
