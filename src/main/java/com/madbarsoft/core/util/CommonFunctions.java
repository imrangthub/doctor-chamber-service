...
import com.madbarsoft.core.pagination.DataTableResults;
...
public interface CommonFunctions {
	...
	default String buildStr(String str, Map<String,Object> map) {
		...
		for (Map.Entry<String,Object> entry : map.entrySet()) {
			if( entry.getValue() instanceof String) {
				replaceStr = replaceStr.replaceAll(entry.getKey().toString(),strsingleQuotation(entry.getValue().toString()) );
			}else{
  ...
  default <T> T objectMapperReadValue(String content, Class<T> valueType)
		...
		try {
			...
			return (T) objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).readValue(content, valueType);
     ...
     default <T> DataTableResults<T> dataTableResults(DataTableRequest<T> dtr, List<T> pFilterList, List<T> pList , Long totalRecord) {
	...
	default <T> List<T> objectMapperReadArrayValue(String mapperArrStr, Class<T> clazz) {
		...
		try {
			...
			return  (List<T>) objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false).readValue(mapperArrStr, TypeFactory.defaultInstance().constructCollectionType(List.class, clazz));
...
/*	default List<PrescriptionDetailsEntity> getDetailList(List<PrescriptionDetailsEntity> presDetailList, int dataTypeid ){
		   
		  return presDetailList.stream().filter(pd -> pd.getPrescritionDataType() == dataTypeid).collect(Collectors.toList());     
			  }
